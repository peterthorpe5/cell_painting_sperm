...
import numpy as np
import pandas as pd
# Dose–response fitting using the `dose-response` package
from dose_response.curve_fitting import FourPLFitter
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fit a 4PL dose–response curve using the dose-response package" 
    )
    parser.add_argument(
        "--min_drc_points",
        dest="min_drc_points",
...
def fit_dose_response_per_compound(
    *,
    ...
    """
    Fit four-parameter logistic curves using the `dose-response` package.

    Uses AR_pct_compound as the response. Fitting is only attempted for
    compounds with at least `min_points` unique concentrations > 0.

    Parameters
    ----------
    fisher_df : pandas.DataFrame
        Output of Fisher testing (one row per compound–dose).
    min_points : int
        Minimum number of distinct doses required for a fit.
    ...
    """
    LOGGER.info(
        "Fitting dose–response models using FourPLFitter (min %d dose levels).",
        min_points,
    )
    ...
    for cpd_id, group in fisher_df.groupby("cpd_id"):
        g = group[group["conc"] > 0].sort_values("conc").copy()
        doses = g["conc"].unique()

        if len(doses) < min_points:
            LOGGER.debug(
                "Skipping %s: only %d distinct doses.",
                cpd_id, len(doses)
            )
            continue
        ...
        try:
            model = fitter.fit(conc, resp)
            params = model.params_

            ec50 = params["EC50"]
            log_ec50 = math.log10(ec50) if ec50 > 0 else np.nan
            ...
            record = {
                ...
                "top": params["Top"],
                "hill": params["HillSlope"],
                "max_delta_AR_pct": float(group["delta_AR_pct"].max()),
                "fit_success": True,
            }

        except Exception as e:
            LOGGER.warning("DRC fit failed for %s: %s", cpd_id, str(e))
            record = {
                "cpd_id": cpd_id,
                "n_doses": len(doses),
                "ec50": np.nan,
    ...
    LOGGER.info("Dose–response fits completed for %d compounds.", df.shape[0])
    return df
...
def plot_dose_response_examples(
    *,
    ...
    drc_df: pd.DataFrame,
    output_dir: Path,
    max_compounds: int = 24,
) -> list[Path]:
    """
    Plot fitted 4PL dose–response curves (from `dose-response` package).
    """
    if drc_df.empty:
        LOGGER.info("No dose–response fits to plot.")
        return []
    ...
    for _, row in subset.iterrows():
        ...
        # Recreate the model from stored parameters
        params = {
            "Bottom": row["bottom"],
            "Top": row["top"],
            "EC50": row["ec50"],
        ...
        model = FourPLFitter().create_model_from_params(params)

        x_dense = np.linspace(conc.min(), conc.max(), 400)
        y_dense = model.predict(x_dense)

        # Plot
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.scatter(conc, resp, s=40, label="Observed")
        ax.plot(x_dense, y_dense, label="4PL fit")

        ax.set_xscale("log")
        ax.set_xlabel("Concentration")
        ax.set_ylabel("AR%")
        ax.set_title(f"{cpd_id} – EC50={row['ec50']:.3g}")
        ...
        LOGGER.info("Saved DRC plot for %s", cpd_id)
